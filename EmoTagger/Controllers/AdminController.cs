...
namespace EmoTagger.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            // En az dinlenen müzikler - sayfalama ile
            var leastPlayedQuery = musicPlayCounts
                .OrderBy(x => x.Count)
                .ThenBy(x => x.MusicId);

            var totalLeastPlayed = leastPlayedQuery.Count();
            var pageSize = 15;
            var totalPages = (int)Math.Ceiling(totalLeastPlayed / (double)pageSize);
            var currentPage = 1;

            var pagedLeastPlayed = leastPlayedQuery
                .Skip((currentPage - 1) * pageSize)
                .Take(pageSize)
                .ToList();

            var leastPlayedIds = pagedLeastPlayed.Select(lp => lp.MusicId).ToList();
            ...
            ViewBag.LeastPlayedList = leastPlayedMusicEntities
                .Select(m => new {
                    title = m.title,
                    artist = m.artist,
                    Count = pagedLeastPlayed.First(lp => lp.MusicId == m.musicid).Count
                })
                .OrderBy(m => m.Count)
                .ThenBy(m => m.title)
                .ToList();

            ViewBag.LeastPlayedCurrentPage = currentPage;
            ViewBag.LeastPlayedTotalPages = totalPages;
        ...
        public async Task<IActionResult> GetLeastPlayedTracks(int page = 1, int pageSize = 15)
        {
            if (!await IsAdminAsync())
                return RedirectToAction("Login", "Dashboard");
            ...
            var totalCount = musicPlayCounts.Count;
            var totalPages = (int)Math.Ceiling(totalCount / (double)pageSize);
            var pagedPlayCounts = musicPlayCounts
                .Skip((page - 1) * pageSize)
                .Take(pageSize)
                .ToList();

            var leastPlayedIds = pagedPlayCounts.Select(lp => lp.MusicId).ToList();
            var leastPlayedMusicEntities = await _context.Musics
                .Where(m => leastPlayedIds.Contains(m.musicid))
                .ToListAsync();

            var result = leastPlayedMusicEntities
                .Select(m => new {
                    title = m.title,
                    artist = m.artist,
                    Count = pagedPlayCounts.First(lp => lp.MusicId == m.musicid).Count
                })
                .OrderBy(m => m.Count)
                .ThenBy(m => m.title)
                .ToList();

            ViewBag.LeastPlayedList = result;
            ViewBag.LeastPlayedCurrentPage = page;
            ViewBag.LeastPlayedTotalPages = totalPages;

            return PartialView("_LeastPlayedTracks", result);
        }

        [HttpGet]
        public IActionResult AnalyzeSong(int musicid)
        {
            var music = _context.Musics.FirstOrDefault(m => m.musicid == musicid);
            if (music == null)
                return Json(new { success = false, message = "Şarkı bulunamadı." });

            string url = $"https://emomusicc.vercel.app/music/{music.filename}";

            var psi = new ProcessStartInfo
            {
                FileName = "python",
                Arguments = $"PythonScript/analyze_song.py \"{url}\"",
                RedirectStandardOutput = true,
                UseShellExecute = false,
                CreateNoWindow = true
            };
